...
import {
  ...
  Sparkles, CheckCircle2, UserCheck, LayoutDashboard, Heart, Settings, Clock, Database, Award
} from 'lucide-react';
...
interface SidebarProps {
  ...
  onToggle: () => void;
  user: User | null;
  activeTab: 'dashboard' | 'master' | 'discipline' | 'scan' | 'reports' | 'import' | 'settings';
  onSelectTab: (tab: 'dashboard' | 'master' | 'discipline' | 'scan' | 'reports' | 'import' | 'settings', subTab?: 'students' | 'teachers' | 'classes' | 'guardians') => void;
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 p-3 space-y-2 overflow-y-auto">
          ...
          {/* 5. PENGATURAN JAM & HARI LIBUR MENU */}
          {user.role === 'admin' && (
            <button
              ...
              title="Pengaturan Jam & Hari Libur"
            >
              <Clock className="w-5 h-5 text-amber-300 shrink-0" />
              {isOpen && <span className="truncate">Pengaturan Hari Libur & Jam</span>}
            </button>
          )}
        </nav>
